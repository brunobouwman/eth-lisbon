...
async function fetchData(accessToken, type, dataSourceId) {
    ...
    const dataArray = [];
    for (let b = 0; b < rawData.bucket.length; b++) {  // Iterate through each bucket
        const bucket = rawData.bucket[b];
        for (let d = 0; d < bucket.dataset.length; d++) {  // Iterate through each dataset in the current bucket
            const dataset = bucket.dataset[d];
            for (let p = 0; p < dataset.point.length; p++) {  // Iterate through each point in the current dataset
